"""
loader.py — Python wrapper for VeloSpec CUDA kernels.

Tries to import the compiled CUDA extension. Falls back to Python reference
implementations if kernels are not built (e.g., on CPU-only machines).

Usage:
    from velospec.kernels.loader import (
        is_cuda_available,
        popcount_density,
...
try:
    ...
    velospec_kernels = None  # type: ignore
...
def popcount_density(
    ...
    if _CUDA_AVAILABLE and torch.cuda.is_available():
        ...
        velospec_kernels.popcount_density(bitmask_gpu, total, num_words)
        count = total.item()
...
def grammar_masked_argmax(
    ...
    Args:
        ...
        bitmask: [num_words] for single, or [K+1, num_words] for batch
        vocab_size: total vocabulary size
    ...
    if logits.dim() == 2:
        ...
        if _CUDA_AVAILABLE and torch.cuda.is_available():
            velospec_kernels.grammar_masked_argmax(
